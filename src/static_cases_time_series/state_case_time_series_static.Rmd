...
plot_time <- function(state, window_num, y_axis_max){
  ...
  # get events with lines - 15M, vaccine, labor day
  ...
  annotation3 <- data.frame(
    x=state_event2$Date,
    y=rep(y_axis_max-80, length.out=nrow(state_event2)),
    label=state_event2$Event2
  )
  #vaccine data separately
  annotation33 <- data.frame(
    x=state_event_vac$Date,
    y=rep(y_axis_max-120, length.out=nrow(state_event_vac)),
  ...
  p <- ggplot(state_data, aes(Date, `New Cases/500,000 ppl`)) +
    annotate("rect", xmin = stay_home_date, xmax = stay_home_end_date,
         ymin = -15, ymax = y_axis_max-65, alpha = .05)+
    ...
    geom_segment(data=state_event2,
               aes(x=Date, y=-15, xend=Date,
                   yend=y_axis_max-100), size=0.1, alpha=0.7)+
    geom_segment(data=state_event_vac,
               aes(x=Date, y=-15, xend=Date,
                   yend=y_axis_max-135), size=0.1, alpha=0.7)+
    ylim(-15, y_axis_max)+
    theme(legend.title=element_blank())+
    labs(x = "Months", y = "New Daily Cases per 500,000 people",
...
plotname = paste(tolower(state)," staticcasetimeseries.png",sep="")
