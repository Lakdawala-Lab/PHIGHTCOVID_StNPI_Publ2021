...
# change colum names
colnames(data) <- c("State", "Date", "New_Confirmed", "New_Deaths", "Event_Catg_S","Score", "Short_Description")
...
rescale data into 1,000,000 normalized
```{r}
...
text_y_pos <- function(state_event, days_diff, space_decs,
  ...
  y_pos_vec = c(max(
    state_event[,"New Deaths/1,000,000 ppl"])+label_position_offset)
  for (i in 2:nrow(state_event)){
    ...
    if (as.integer(diff)<days_diff){
      ...
      y_pos_vec = append(y_pos_vec,
                         max(state_event[,"New Deaths/1,000,000 ppl"])+
                           label_position_offset)
    }
  }
  return(y_pos_vec)
...
plot_time_interact <- function(state, window_num, days_diff, space_desc,
  ...
  p <- ggplot(state_data, aes(Date, `New Deaths/1,000,000 ppl`,
                              label1=Date)) +
    geom_point(aes(label2=`New Deaths/1,000,000 ppl`),size=2, alpha=1) + 
    ...
    labs(x = "Months", y = "New Daily Deaths Normalized")+
...
htmlwidgets::saveWidget(as_widget(p),
                        paste(tolower(state),"_deathInteractive.html",sep=""))
```
